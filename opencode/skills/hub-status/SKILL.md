---
name: hub-status
description: Show the MCP Task Hub queue — counts, blocked tasks with reasons, stale claims — grouped by project and change. Use when the user asks "hub status", "what's in the queue", "any blocked tasks", or after a drain run. The browser view is http://taskhub.local/ui (LAN); its Specs screen lists changes pending human review.
---

# hub-status — queue overview

Fast path: `python3 ~/.config/opencode/scripts/hub/hub-cli.py status` prints hub health,
