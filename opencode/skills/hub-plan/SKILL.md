...
You (the primary agent, normally `build` or `plan`) are the planner. You never implement here. The output is
...
For more than a handful of tasks, delegate the sync to the `hub-steward`
subagent — "Job D" — with the project name and the full task list as JSON;
it calls `task-hub_sync_task` per task verbatim. If the `task-hub_*` tools
are missing (the `mcp.task-hub` entry is not in opencode.json), use
`~/.config/opencode/scripts/hub/hub-cli.py` — a plain-HTTP MCP client:
   `echo '<tasks JSON array>' | python3 ~/.config/opencode/scripts/hub/hub-cli.py sync-batch --project <p>`
...
fan the §2 audit out: one `explore` subagent per tasks.md section, each classifying
